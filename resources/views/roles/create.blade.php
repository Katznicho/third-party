...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-slate-600 mt-1 text-sm">Define a role and assign permissions.</p>
        </div>
    ...
    <form method="POST" action="{{ route('roles.store') }}" class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}"
                       placeholder="e.g. Claims Officer, Finance Manager"
                    ...
                    <div class="border border-slate-200 rounded-lg p-4">
                        <label class="inline-flex items-center mb-2">
                            <input type="checkbox" name="permissions_menu[]" value="{{ $group }}"
                                   ...
                                   {{ in_array($group, old('permissions_menu', [])) ? 'checked' : '' }}>
                            ...
                            <div class="pl-5 mt-2 border-l border-slate-200">
                                <label class="inline-flex items-center mb-1">
                                    <input type="checkbox" name="permissions_menu[]" value="{{ $category }}"
                                           ...
                                           {{ in_array($category, old('permissions_menu', [])) ? 'checked' : '' }}>
                                    ...
                                    @foreach ($perms as $permission)
                                        <label class="inline-flex items-center space-x-2 mr-4">
                                            <input type="checkbox" name="permissions_menu[]" value="{{ $permission }}"
                                                   ...
                                                   {{ in_array($permission, old('permissions_menu', [])) ? 'checked' : '' }}>
