...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-slate-600 mt-1 text-sm">{{ $department->name }}</p>
        </div>
        <a href="{{ route('departments.index') }}" class="px-4 py-2 bg-slate-600 text-white rounded-lg text-sm font-medium hover:bg-slate-700">
            ← Back
        </a>
    </div>
        ...
        <form action="{{ route('departments.update', $department) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" maxlength="1000"
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $department->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex justify-end gap-3 pt-4 border-t border-slate-200">
                <a href="{{ route('departments.index') }}" class="px-4 py-2 border border-slate-300 text-slate-700 rounded-lg text-sm hover:bg-slate-50">Cancel</a>
