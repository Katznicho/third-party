                    ...
                    <div class="border border-slate-200 rounded-lg p-4 product-item">
                        ...
                        <div id="fields_{{ $category->id }}" class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3" style="display: {{ $isChecked ? 'grid' : 'none' }};">
                            <div>
                                ...
                                <input type="number" name="service_categories[{{ $category->id }}][benefit_amount]" step="0.01" min="0" value="{{ old('service_categories.' . $category->id . '.benefit_amount', $pivot->pivot->benefit_amount ?? '') }}" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0.00">
                            </div>
                            
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-2">Co-pay Percentage (%)</label>
                                <input type="number" name="service_categories[{{ $category->id }}][copay_percentage]" step="0.01" min="0" max="100" value="{{ old('service_categories.' . $category->id . '.copay_percentage', $pivot->pivot->copay_percentage ?? 0) }}" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0">
                            </div>
                            
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-2">Deductible Amount (UGX)</label>
                                <input type="number" name="service_categories[{{ $category->id }}][deductible_amount]" step="0.01" min="0" value="{{ old('service_categories.' . $category->id . '.deductible_amount', $pivot->pivot->deductible_amount ?? 0) }}" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0.00">
                            </div>
                            
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-2">Waiting Period (Days)</label>
                                <input type="number" name="service_categories[{{ $category->id }}][waiting_period_days]" min="0" value="{{ old('service_categories.' . $category->id . '.waiting_period_days', $pivot->pivot->waiting_period_days ?? ($category->waiting_period_days ?? 0)) }}" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0">
                            </div>
