    ...
    <form action="{{ route('plans.update', $plan) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-6">
        ...
        <div class="border-b border-slate-200 pb-4">
            ...
            <p class="text-xs text-slate-500 mb-2">
                <strong>Coverage %</strong> applies to every line in that service category on a visit and overrides connected-provider item % when below 100%.
            </p>
                ...
                @foreach($serviceCategories as $category)
                    ...
                    <div class="border border-slate-200 rounded-lg p-4 product-item {{ $isStandard ? 'bg-blue-50 border-blue-300' : '' }}">
                        ...
                        <div id="fields_{{ $category->id }}" class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3" style="display: {{ $isChecked ? 'grid' : 'none' }};">
                            ...
                            @php
                                $covPct = (float) old('service_categories.' . $category->id . '.coverage_percent', $pivot->pivot->coverage_percent ?? 100);
                            @endphp
                            <div>
                                ...
                                <div class="flex items-center gap-1">
                                    <input
                                        ...
                                        value="{{ $covPct == (int) $covPct ? (int) $covPct : number_format($covPct, 2, '.', '') }}"
                                        min="0"
                                        max="100"
                                        step="0.01"
