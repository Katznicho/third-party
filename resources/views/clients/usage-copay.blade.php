...
@section('title', 'Co-pay Usage')
@section('page-title', 'Co-pay Usage')

@section('content')
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Co-pay Usage</h1>
            <p class="text-slate-600 mt-1">
                {{ $client->full_name }} — Total co-pay collected:
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-semibold text-slate-600 uppercase">Co-pay (UGX)</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-200">
                    @forelse($authorizations as $auth)
                        ...
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3 text-sm text-right text-blue-700">{{ number_format($breakdown['copay'] ?? 0, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center text-sm text-slate-500">
                                No co-pay usage found for this client.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
