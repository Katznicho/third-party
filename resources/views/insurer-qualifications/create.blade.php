...
@section('page-title', 'Create qualification')

@section('content')
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">New qualification</h1>
            <p class="text-slate-600 mt-1 text-sm">Add a staff qualification for your insurance company.</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 max-w-xl">
        <form action="{{ route('qualifications.store') }}" method="POST" class="space-y-5">
            @csrf
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required maxlength="255"
                    ...
                    placeholder="e.g. Bachelor of Commerce, CPA">
                @error('name')
